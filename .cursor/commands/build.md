...
|-------------------|--------|
| `automate_human` / `automate_adb` | Run `bash scripts/attempt-build-plan-row.sh --owner "<owner>" --task "<task>" --sprint "<sprint>" --json`. On exit 0: replace 🔲 → ✅ for that row in BUILD_PLAN.md. On exit 1: `bash scripts/build-backlog.sh add --owner "<owner>" --task "<task>" --sprint "<sprint>" --reason "<reason from JSON>"` and **continue** (row stays open). |
...
| `parallel_dispatch` | Run @.cursor/commands/scope.md fully, then `bash scripts/agent-progress.sh set-parallel-sprint-done --sprint "<sprint title>"` |
